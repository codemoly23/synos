...
export const config = {
  api: {
    bodyParser: {
      sizeLimit: '25mb', // Allow for base64 overhead
    },
  },
};
```
...
```typescript
// Service generates URL
const url = `/storage/${folder}/${filename}`;

// Full URL (for API responses)
...
```typescript
// Future enhancement: Rate limit uploads per user/IP
...
```typescript
// Optional: Virus scanning before saving
...
```typescript
// If subsequent operations fail, clean up uploaded file
try {
  const file = await storageService.upload(file);
  await saveToDatabase(file.url); // If this fails...
} catch (error) {
  await storageService.delete(file.filename, file.folder); // ...clean up
  throw error;
}
```
...
```typescript
// Consider using sharp for image optimization
...
### 4. Batch Operations
```typescript
// For multiple file uploads, process in parallel
...
```typescript
// Future: Storage adapter interface
...
```tsx
// After upload, use the URL directly
<img src={uploadedFile.url} alt="Uploaded image" />

// Or with Next.js Image (add domain to next.config.ts)
