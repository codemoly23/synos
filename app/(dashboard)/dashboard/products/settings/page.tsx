				...
				if (res.ok) {
					...
					form.reset({
						heroSection: {
							...
							bulletPoints: Array.isArray(data?.heroSection?.bulletPoints) ? data.heroSection.bulletPoints : [],
				...
				form.reset({
					...
					faqSection: {
						...
						faqs: (updated?.faqSection?.faqs ?? []).map(
							(f: { _id?: string; question?: string; answer?: string; visible?: boolean; order?: number }) => ({
								_id: f._id?.toString(),
								question: f.question ?? "",
								answer: f.answer ?? "",
								visible: f.visible ?? true,
					...
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<p className="text-muted-foreground">
								Manage the hero and FAQ sections shown on the Produkter page.
							</p>
						</div>
					</div>
					<a
				...
				>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Controls the hero banner at the top of the Produkter page. Leave fields empty to use the default hardcoded values.
							</CardDescription>
						</CardHeader>
