...
export default function CategoriesSettingsPage() {
	...
	React.useEffect(() => {
		if (!isPending && !session) {
			router.push("/login?callbackUrl=/dashboard/categories/settings");
		}
	...
	React.useEffect(() => {
		...
		async function fetchData() {
			try {
				const res = await fetch("/api/klinikutrustning-page");
				if (res.ok) {
				...
				>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Title displayed above the FAQ list on the Klinikutrustning page.
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-4">
							<div className="space-y-2">
								...
								<Input
									...
									placeholder="Vanliga frågor om klinikutrustning"
								/>
							</div>
						</CardContent>
					</Card>
									...
									<div className="flex-1 space-y-4">
										<div className="space-y-2">
											...
											<Input
												...
												placeholder="Vilken laser passar bäst för min klinik?"
											/>
										</div>
										<div className="space-y-2">
