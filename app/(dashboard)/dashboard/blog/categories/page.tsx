...
/**
 * TreeNode Component - Renders a single blog category in the tree
 */
function TreeNode({
	node,
	onEdit,
...
/**
 * Blog Categories Dashboard Page
 */
export default function BlogCategoriesPage() {
	...
	React.useEffect(() => {
		if (!isPending && !session) {
			router.push("/login?callbackUrl=/dashboard/blog/categories");
		}
	...
	const countCategories = (nodes: IBlogCategoryTreeNode[]): number => {
	...
	const handleEdit = (id: string) => {
		router.push(`/dashboard/blog/categories/${id}`);
	};
	...
	const handleDelete = async (id: string) => {
		...
		try {
			...
			if (response.ok) {
				...
				const data = await response.json();
				toast.error(data.message || "Failed to delete blog category");
			}
		} catch (error) {
	...
	const handleToggleActive = async (id: string) => {
		try {
			...
			console.error("Failed to toggle blog category:", error);
		}
	};

	// Find node by ID in tree
	const findNodeById = (
		nodes: IBlogCategoryTreeNode[],
		id: string
	): IBlogCategoryTreeNode | null => {
		for (const node of nodes) {
		...
		return (
			<div className="space-y-6">
				<div className="flex justify-between items-center">
					<div>
						<div className="h-9 w-48 bg-accent animate-pulse rounded-md mb-2" />
			...
			<div className="space-y-6">
				...
				<Card>
					<CardContent className="pt-6">
						...
						<p className="text-sm text-slate-600">Total Blog Categories</p>
					</CardContent>
				</Card>
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Organize your blog categories in a hierarchical structure
						</CardDescription>
					</CardHeader>
					<CardContent>
						{isLoading ? (
