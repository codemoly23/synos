...
export function TrainingInquiryForm() {
	...
	const {
		...
		defaultValues: {
			...
			phone: "",
			trainingInterestType: undefined,
			corporationNumber: "",
			message: "",
	...
	const handleInterestTypeChange = (value: string) => {
		...
		setValue(
			...
			value as (typeof trainingInterestTypes)[number]
		);
	};
	...
	return (
		...
		>
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
				...
				{/* Right Column - Training Interest */}
				<div className="space-y-6">
					<h3 className="text-lg font-semibold text-secondary flex items-center gap-2 pb-2 border-b border-border">
					...
					{/* Training Interest Type Radio Group */}
					<div className="space-y-3">
						...
						<RadioGroup
							...
							className="space-y-2"
						>
							{trainingInterestTypes.map((type) => (
								<div
									key={type}
									className={cn(
									...
									onClick={() => handleInterestTypeChange(type)}
								>
									<RadioGroupItem
										value={type}
										id={type}
									...
									>
										{trainingInterestTypeLabels[type]}
									</Label>
								</div>
							))}
						</RadioGroup>
						{errors.trainingInterestType && (
							<p className="text-sm text-red-500">
								{errors.trainingInterestType.message}
							</p>
						)}
					</div>
				</div>
